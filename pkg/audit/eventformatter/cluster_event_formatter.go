...
func (f *clusterEventFormatter) getFormattedDetailsClusterUpdated(eventData *eventdata.ClusterUpdated) string {
	...
	oldCluster, err := f.getClusterById(f.ctx, f.event.AggregateId)
	if err != nil {
		return ""
	}
...
func (f *clusterEventFormatter) getFormattedDetailsClusterDeleted() string {
	cluster, err := f.getClusterById(f.ctx, f.event.AggregateId)
	if err != nil {
		return ""
	}

	return fmt.Sprintf("%s deleted %s", f.event.Metadata["x-auth-email"], cluster.DisplayName)
}