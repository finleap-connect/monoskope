...
func (b *rabbitEventBus) PublishEvent(ctx context.Context, event events.Event) *messageBusError {
...
// recieve the message.
func (b *rabbitEventBus) publishEvent(event events.Event) *messageBusError {
	if !b.isConnected {
		return &messageBusError{
			Err: ErrMessageNotConnected,
		}
...
func (b *rabbitEventBus) generateRoutingKey(event events.Event) string {
...
func (b *rabbitEventBus) handleIncomingMessages(qName string, msgs <-chan amqp.Delivery, receiver EventReceiver) {
	...
	for d := range msgs {
		...
		err = receiver(events.NewEvent(re.EventType, re.Data, re.Timestamp, re.AggregateType, re.AggregateID, re.AggregateVersion))
		if err != nil {
			if err := d.Nack(false, false); err != nil {
				b.log.Error(err, "Failed to NACK event.")
			}
